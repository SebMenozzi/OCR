...
{
  Matrix* mat = new_matrix(img->h, img->w);
...
{
  Matrix* new_mat = new_matrix(height, width);

  double x_ratio = mat->cols / (double) width;
  double y_ratio = mat->rows / (double) height;
  double px, py;
  for (size_t i = 0; i < height; ++i)
  {
    for (size_t j = 0; j < width; ++j)
    {
      ...
      new_mat->values[(i * width) + j] = mat->values[(size_t) ((py * mat->cols) + px)];
    }
  }
  return new_mat;
}
...
void load_matrix(Matrix* mat, char* directory, char* name)
{
  ...
  if (!file)
  {
      printf("Failure to open the matrix file : %s in directory : %s\n", name, directory);
      exit(1);
  }
  else
  {
